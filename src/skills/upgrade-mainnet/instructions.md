...
Apply `../shared/upgrade-runbook.md` and onboard both instances using `../inventory/instructions.md`. This procedure requires a verified compatible Firedancer/Frankendancer primary and Agave backup for the same vote account. There are no default host pairs or identities. Do not apply it to an Agave primary or a deployment whose identity/tower compatibility is unverified.

For a voting Agave/Jito-Agave primary, use [upgrade-agave-primary](../upgrade-agave-primary/instructions.md).
...
Apply [mainnet handoff readiness](../shared/mainnet-failover.md) in both directions, including loaded voter verification before demotion and the receiver's local replay check against the final transferred tower before promotion. If the backup is borrowed from another validator, use [role switching and restoration](../shared/backup-role-switch.md).
