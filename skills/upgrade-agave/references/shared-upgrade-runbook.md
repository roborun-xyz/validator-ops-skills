...
Use the exact verified release tag/ref rather than constructing a tag from a version string or assuming a fork's tag scheme. For a mainnet identity handoff, apply [readiness and tower verification](shared-mainnet-failover.md); for a borrowed backup, also apply [role switching and restoration](shared-backup-role-switch.md).
