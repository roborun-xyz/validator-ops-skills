...
Apply [upgrade preparation](references/shared-upgrade-runbook.md) and [mainnet identity handoff](references/shared-mainnet-failover.md). This workflow requires an Agave/Jito-Agave primary and a verified compatible Agave/Jito-Agave backup for the same vote account. For a Firedancer primary, use [upgrade-mainnet](references/upgrade-mainnet.md); for an unstaked backup upgrade, use [upgrade-agave](references/upgrade-agave.md).

## Prepare before failover

Read `VALIDATOR_OPS_HOST_INVENTORY` or `~/.config/validator-ops/hosts.md`, respecting an explicit operator path. Verify missing or stale facts through [inventory](references/inventory.md). There are no default hosts, identities, key paths or supervisors.
...
4. If the backup must change duties, use [backup role switching](references/shared-backup-role-switch.md) to prepare the target role, coverage window and restoration. Serialize all operations sharing that backup. Complete preparation before requesting any missing execution authorization; configuration alone grants none.
