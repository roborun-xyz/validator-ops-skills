...
For participation-only checks, create a versioned file with `hosts: []`. For live checks, use [inventory](../SKILL.md) to verify SSH aliases, local RPC ports, client families, expected identities, optional vote accounts, and descriptive roles, then map those facts into this schema. Ask only for missing fields; there are no built-in host aliases or operator identities.
