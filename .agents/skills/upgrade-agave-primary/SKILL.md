...
Apply [upgrade preparation](../shared/upgrade-runbook.md) and [mainnet identity handoff](../shared/mainnet-failover.md). This workflow requires an Agave/Jito-Agave primary and a verified compatible Agave/Jito-Agave backup for the same vote account. For a Firedancer primary, use [upgrade-mainnet](../upgrade-mainnet/SKILL.md); for an unstaked backup upgrade, use [upgrade-agave](../upgrade-agave/SKILL.md).

## Prepare before failover

Read `VALIDATOR_OPS_HOST_INVENTORY` or `~/.config/validator-ops/hosts.md`, respecting an explicit operator path. Verify missing or stale facts through [inventory](../inventory/SKILL.md). There are no default hosts, identities, key paths or supervisors.
